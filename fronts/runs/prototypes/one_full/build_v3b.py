...
from dbof.utils.subset_config import x_build_global_job_config
from dbof.cli import generate_global_depth
...
def main(flg: str):
    ...
    config_file = './testing_global_v3b.yaml'
    ...
    if flg == 0:
        # Load config
        config = x_build_global_job_config(config_file)
        #print(config)
        # Generate global depth
        #generate_global_depth.main(config_file, subset='frontal_structure')
        embed(header='build_v3b.py: 36')
    ...
    if flg == 3:
        config = x_build_global_job_config(config_file)
        find_gradb2_fronts(timestamp, config)
